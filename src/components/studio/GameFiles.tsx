...
export function GameFiles() {
 ...
 return <section className="game-files" aria-label={t('studio.games')}><div className="button-row"><button className="btn secondary" disabled={saving||!game.history.length} onClick={()=>void save()}><Save size={16}/>{t('studio.saveGame')}</button><button className="btn secondary" onClick={()=>downloadText(`chessy-${localDay()}.pgn`,exportPgn(),'application/x-chess-pgn')}><Download size={16}/>{t('studio.exportPgn')}</button><label className="btn secondary file-button"><Upload size={16}/>{t('studio.importPgn')}<input type="file" accept=".pgn,text/plain,application/x-chess-pgn" aria-label={t('studio.importPgn')} onChange={async e=>{const file=e.target.files?.[0];e.target.value='';if(!file)return;try{if(file.size>100000)throw new Error('size');const pgn=await file.text();const check=new Chess();check.loadPgn(pgn);if(!check.history().length)throw new Error('empty');setPending(pgn);setMessage('');}catch{setMessage('studio.pgnError');}}}/></label></div>{pending&&<div className="confirm-panel"><p>{t('studio.pgnReplace')}</p><div className="button-row"><button className="btn primary" onClick={()=>{game.loadPgn(pending);game.setActiveGameId(null);setPending(null);}}>{t('studio.importPgn')}</button><button className="btn quiet" onClick={()=>setPending(null)}>{t('studio.cancel')}</button></div></div>}{message&&<p className="feedback" role="status">{t(message)}</p>}</section>;
}
