...
function PuzzlePlayer({puzzle,onNext,hasNext}:{puzzle:Puzzle;onNext:()=>void;hasNext:boolean}) {
 ...
 return <div className="challenge-layout">
  ...
  <section className="panel challenge-panel" aria-label={t('studio.training')}>
   <div className="icon-tile"><ChessyIcon name="target" size={25}/></div>
   ...
   <div className={`feedback ${session.solved?'success':session.feedback==='wrong'||session.feedback==='illegal'?'wrong':''}`} role="status" aria-live="polite">
    {session.solved&&<ChessyIcon name="check" size={22}/>}<span>{t(`studio.feedback.${session.feedback}`)}</span>
   </div>
   {session.solved&&<div className="success-explanation"><ChessyIcon name="spark" size={20}/><p>{assisted?t('studio.assisted'):t(previouslySolved?'studio.alreadySolved':'studio.earned')}</p></div>}
   ...
   <div className="button-row">
    {!session.solved&&<button className="btn secondary" disabled={hints>=2} onClick={()=>{setHints(h=>h+1);setAssisted(true);}}><ChessyIcon name="hint" size={17}/>{t('studio.hint')}</button>}
   ...
   {hasNext&&<button className="btn primary full" onClick={onNext} disabled={!session.solved&&!revealed}>{t('studio.next')}<ChessyIcon name="arrow" size={18}/></button>}
  </section>
 </div>;
}
