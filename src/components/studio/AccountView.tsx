...
export function AccountView() {
  ...
  if (status === 'signed-in' && user) return <div className="view-enter"><SectionHeading eyebrow={t('studio.account')} title={t('auth.title')} subtitle={t('auth.signedInSubtitle')}/><div className="account-grid"><section className="panel account-profile"><div className="account-avatar" aria-hidden="true">{user.photoUrl?<img src={user.photoUrl} alt="" referrerPolicy="no-referrer"/>:<ChessyIcon name="profile" size={34}/>}</div><div><p className="eyebrow">{t('auth.signedInAs')}</p><h2>{user.displayName || user.email || t('auth.player')}</h2>{user.email&&<p className="muted">{user.email}</p>}<p className="fine-print">{t('auth.providerLabel')}: {t(`auth.provider.${user.provider}`)}</p></div></section><section className="panel account-actions"><h2>{t('auth.accountActions')}</h2><p className="muted">{t('auth.progressLocal')}</p><button className="btn secondary" disabled={busy} onClick={()=>signOut()}>{t('auth.signOut')}</button><div className="account-danger"><h3>{t('auth.deleteTitle')}</h3><p className="fine-print">{t('auth.deleteBody')}</p>{!confirmDelete?<button className="btn quiet" disabled={busy} onClick={()=>setConfirmDelete(true)}>{t('auth.deleteAccount')}</button>:<div className="button-row"><button className="btn danger" disabled={busy} onClick={()=>deleteCurrentAccount()}>{t('auth.confirmDelete')}</button><button className="btn quiet" disabled={busy} onClick={()=>setConfirmDelete(false)}>{t('auth.cancel')}</button></div>}</div></section></div>{error&&<p className="feedback wrong" role="alert">{t(`auth.errors.${error}`)}</p>}{notice&&<p className="feedback success" role="status">{t(`auth.notices.${notice}`)}</p>}</div>;
