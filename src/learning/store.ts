...
interface LearningStore {
 ...
 complete:(kind:'puzzle'|'lesson',id:string)=>void;mistake:(id:string)=>void;importProgress:(raw:string)=>void;replaceProgress:(progress:Progress)=>void;
...
export const useLearningStore=create<LearningStore>((set,get)=>({
 ...
 replaceProgress:incoming=>{const progress=parseProgress(JSON.stringify(incoming));if(!persist(progress))throw new Error('Storage unavailable');set({progress,storageWarning:false});},
